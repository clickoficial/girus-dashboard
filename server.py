...
def etapa_pedido(pedido, info):
    """
    Le a etapa/situacao do pedido. No Omie a etapa fica no cabecalho
    (campo 'etapa') ou as vezes no infoCadastro. Retorna string.
    Etapas comuns: 10=pedido/lancado, 20=aguardando, 50=faturado,
    60=concluido. O relatorio Andy filtra 'Autorizado' (faturado/efetivado).
    """
    cab = pedido.get("cabecalho", {}) or {}
    for fonte in (cab, info):
        v = fonte.get("etapa")
        if v not in (None, ""):
            return str(v).strip()
    return ""
...
def data_emissao_pedido(info):
    """
    Le a data de emissao do pedido (infoCadastro) e devolve um datetime
    (sem timezone). Tenta os campos que o Omie usa, em ordem de preferencia.
    Retorna None se nao conseguir interpretar.
    """
    for campo in ("dDtPrevisao", "data_previsao", "dDtFat",
                  "data_faturamento", "dDtRegistro", "data_registro",
                  "dDtInclusao", "data_inclusao"):
    ...
    while True:
        ...
        for pedido in pedidos:
            ...
            if str(info.get("cancelado", "N")).upper() == "S":
                cancelados += 1
                continue
            # So conta o que foi emitido entre o dia 01 e hoje.
            # A API pode devolver pedidos com data futura/agendada — esses
            # ainda nao foram faturados, entao ficam de fora (bate com Andy).
            dt_emis = data_emissao_pedido(info)
